...
//     })
    ...
    describe("Login and Buyer wise color size Test with Create=>", () => {
      it("Login and Buyer wise color Test with create", () => {
        cy.login();
        cy.wait(2000);
        cy.get("input#navbar-search").type("Buyer Wise Color");
        cy.get('li[aria-selected="true"]').first().click();
        ...
        cy.get('input[data-fieldtype="Duration"]').click("right");// Example duration in days
        // cy.get('input[data-duration="hours"]').clear().type("20") // Example duration in days
        // cy.get('input[data-duration="minutes"]').clear().type("30")
        // cy.get('input[data-duration="seconds"]').clear().type("0") // Example duration in hours
