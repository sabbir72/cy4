...
describe("Login and Pre cost print", () => {
  it("Login and Pre cost print", () => {
    cy.login();
    cy.wait(2000);
    ...
    cy.get('input[placeholder="ID"]').eq(0).type('0029-021');
    cy.contains('a', 'Pre Costing-RMG-0029-021').first()
  .should('be.visible')                     
  .click();
  
  cy.get('button[data-original-title="Print"]')
  .should('be.visible')
  .click();
